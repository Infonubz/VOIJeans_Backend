...
exports.VoijeansInvoiceListByInvoiceNo = async (req, res) => {
    try {
        ...
        const groupedProducts = products.reduce((acc, item) => {
            ...
            totalNetAmount += (item.gross_amount || 0) + (item.igst_amount || 0); // Updated logic
        ...
        if (parseInt(voi_advance_request_id) === 8) {
            query = `
                WITH CTE_UniqueInvoices AS (
                    ...
                    GROUP BY invoice_no, voi_branch_name, party_branch_name, invoice_date, in_advance_request_id, in_advance_request, payment_status_id,
                ...
                voi_advance_request, payment_status_id, payment_status,
            ...
            query = `
                WITH CTE_UniqueInvoices AS (
                    SELECT TRIM(invoice_no) AS invoice_no, voi_branch_name, party_branch_name, invoice_date,
                           ...
                           ROW_NUMBER() OVER (PARTITION BY invoice_no ORDER BY invoice_date DESC) AS row_num
                    FROM voi_jeans_invoice_list
                    WHERE voi_advance_request_id = @voi_advance_request_id AND advance_amount IS NOT NULL
                    GROUP BY invoice_no, voi_branch_name, party_branch_name, invoice_date, in_advance_request_id, in_advance_request, payment_status_id,
                             payment_status, adv_paid_date,  request_date, comment, interest_amt, interest_days, advance_amount, advance_percentage, closing_date,
                             credit_period, balance_amount, voi_advance_request, voi_advance_request_id
                )
...
exports.getVoiInvoiceListHsnCode = async (req, res) => {
    try {
        ...
        // Process each record and update the response
        result.recordset.forEach(record => {
            ...
            // Add HSN data to the list
            response.hsn_list.push(hsnData);
        });

        // Convert totals to fixed decimal places, except for total_qty
...
exports.updateRequestStatus = async (req, res) => {
    try {
        ...
        if (in_advance_request_id === 2 || in_advance_request_id === 3 || in_advance_request_id === 4) {
        ...
        if (in_advance_request_id === 2) {
            notificationMessage = `${advancePercentage}% Advance for this invoice ${invoice_no} is Approved`;
        } else if (in_advance_request_id === 3) {
            notificationMessage = `Advance for this invoice ${invoice_no} is on Hold`;
        } else if (in_advance_request_id === 4) {
            notificationMessage = `Advance for this invoice ${invoice_no} is Rejected`;
        }
    ...
    try {
        ...
        const query = `
            SELECT 
                ...
                --adv_paid_date,
                SUM(sales_quantity) AS total_qty,
                SUM(gross_amount) AS total_invoice_amt,
                advance_amount,
                interest_days AS interest_total_days,
                interest_amt,
                (interest_amt / NULLIF(interest_days, 0)) AS interest_per_day_amt,
                ((SUM(gross_amount) - advance_amount - interest_amt)) AS net_amt,
                SUM(mrp) AS total_mrp_amt
            FROM innofashion_invoice_list 
            WHERE voi_invoice_no = @voi_invoice_no
            GROUP BY 
                voi_invoice_no, invoice_date,
                in_invoice_no, 
                debit_no, 
                debit_date, 
                --adv_paid_date, 
...
exports.VoicountAPI = async (req, res) => {
    try {
        ...
        const query = `
                    SELECT 
                    -- Count of distinct invoice numbers based on advance request status
